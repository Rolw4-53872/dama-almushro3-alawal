...
let AuthService = class AuthService {
    ...
    async register(createUserDto) {
        ...
        if (existingUser) {
            throw new common_1.BadRequestException('البريد الإلكتروني مستخدم بالفعل');
        }
    ...
    async login(loginDto) {
        ...
        const refreshToken = this.jwtService.sign({ sub: user.id }, { expiresIn: '7d', secret: process.env.JWT_REFRESH_SECRET });
        return {
            accessToken,
            refreshToken,
            user: {
    ...
    async refreshToken(refreshToken) {
        try {
            ...
            throw new common_1.UnauthorizedException('التوكن غير صحيح');
        }
    }
};
